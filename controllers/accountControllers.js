...
exports.signup = async (req, res, next) => {
  ...
  try {
    const usernameExistedInPatient = await Patient.findOne({
      username: data.username,
    });
    const usernameExistedInTherapist = await Therapist.findOne({
      username: data.username,
    });
    const usernameExistedInAdmin = await Admin.findOne({
      username: data.username,
    });

    if (
      usernameExistedInPatient ||
      usernameExistedInTherapist ||
      usernameExistedInAdmin
    ) {
...
exports.getTherapistRequestList = async (req, res, next) => {
  try {
    const pendingTherapists = await Therapist.find({ status: 'Pending' });
